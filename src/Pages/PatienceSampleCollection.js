...
const PatientSample = () => {
  ...
  const fetchData = async() => {
    const data=  await axios.get(`${API}/getpatiencelist`,{ headers: {"authtoken" :`${TOKEN}`} })
  ...
  <div className={tableclasses.body}>
    <div  className={tableclasses.header}>
      <div className={tableclasses.name}>
        ...
        <div  className={tableclasses.specification}>345 pending sample collection</div>
      </div>
      <div>
    ...
    <div>
      ...
      <Buttons className={tableclasses.filterButton2}>Sample Status</Buttons>
      <Buttons className={tableclasses.filterButton3} >
       ...
       <span style={{color: '#FF8743', marginLeft: 6, text: 'center'}}>Reset FIlter</span> </Buttons>
    ...
    <Table className={tableclasses.table}>
      <TableHead className={tableclasses.tableHead}>
        <TableRow >
          <TableCell className={tableclasses.customHeadName}>Collection name</TableCell>
          <TableCell className={tableclasses.customHeadName}>Location</TableCell>
          <TableCell className={tableclasses.customHeadName}>Patient name & no</TableCell>
          <TableCell className={tableclasses.customHeadName}>Date & Time</TableCell>
          <TableCell className={tableclasses.customHeadName}>Lab No</TableCell>
          <TableCell className={tableclasses.customHeadName}>Reffered by</TableCell>
          <TableCell className={tableclasses.customHeadName}>Sample</TableCell>
        ...
        {rows?.map((row) => (
          <TableRow key={row.id}>
            <TableCell component="th" scope="row" className={tableclasses.customTableCell}>
                  <div  className={tableclasses.name}>
                    {console.log('row?.collector',row?.collector.name.toString())}
            ...
            <div style={{text: 'center',
              ...
              <path d="M8 8.61523L4.2 5.76523C4.13826 5.71833 4.08811 5.65787 4.05344 5.58852C4.01876 5.51916 4.00048 5.44277 4 5.36523V3.11523C4 2.98263 4.05268 2.85545 4.14645 2.76168C4.24021 2.66791 4.36739 2.61523 4.5 2.61523H11.5C11.6326 2.61523 11.7598 2.66791 11.8536 2.76168C11.9473 2.85545 12 2.98263 12 3.11523V5.34023C11.9995 5.41777 11.9812 5.49416 11.9466 5.56352C11.9119 5.63287 11.8617 5.69333 11.8 5.74023L8 8.61523Z" stroke="#D48A48" stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M8 8.61523L4.2 11.4652C4.13826 11.5121 4.08811 11.5726 4.05344 11.642C4.01876 11.7113 4.00048 11.7877 4 11.8652V14.1152C4 14.2478 4.05268 14.375 4.14645 14.4688C4.24021 14.5626 4.36739 14.6152 4.5 14.6152H11.5C11.6326 14.6152 11.7598 14.5626 11.8536 14.4688C11.9473 14.375 12 14.2478 12 14.1152V11.8902C11.9995 11.8127 11.9812 11.7363 11.9466 11.667C11.9119 11.5976 11.8617 11.5371 11.8 11.4902L8 8.61523Z" stroke="#D48A48" stroke-linecap="round" stroke-linejoin="round"/>
              </svg></div>       
              <div><span style={{color: '#D48A48', marginLeft: 6, text: 'center'}}>{row.status}</span></div>
              </div>
            </TableCell>
            <TableCell className={tableclasses.customTableCell}>
